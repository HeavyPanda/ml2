...
\extitle{Exercise 1a}
Let $\vec{x}_1, \ldots, \vec{x}_n \in \mathcal{X}$ be arbitrary samples 
...
\end{gather*}
We could also proof this for all pairs of datapoints $x, x'$ out of $\mathcal{X}$, but then the unfold of the 
...
\underbrace{
\begin{bmatrix}
	\sqrt{\beta_1} \phi_1(x)	& \dots	& \sqrt{\beta_L} \phi_L(x)      
\end{bmatrix}
}_{\phi(x)^\top}
\underbrace{
\begin{bmatrix}
...
\end{gather*}
$\phi(x)^\top$ and $\phi(x')$ are block partitioned matrices, i.e. the $\phi_l's$ are simply concatenated 
together in one very long vector. So, the result is: $\phi(x) = \begin{bmatrix}
	\sqrt{\beta_1} \phi_1(x)	& \dots	& \sqrt{\beta_L} \phi_L(x)      
\end{bmatrix}^\top$.
